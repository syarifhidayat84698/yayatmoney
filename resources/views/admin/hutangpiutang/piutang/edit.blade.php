    ...
    <form enctype="multipart/form-data" method="POST" action="{{ route('piutang.update', $credit->id) }}">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="status" name="status" required>
                <option value="Belum Terbayar" {{ $credit->status == 'Belum Terbayar' ? 'selected' : '' }}>Belum Terbayar</option>
                <option value="Terbayar" {{ $credit->status == 'Terbayar' ? 'selected' : '' }}>Terbayar</option>
            </select>
        </div>
